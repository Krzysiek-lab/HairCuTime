...
public class AppointmentServiceImpl implements AppointmentService {
    ...
    @Override
    public AppointmentDto updateAppointment(CreateAppointmentDto appointmentDto, long id) {
        return appointmentRepository.findById(id).map(appointment -> {
            appointment.setProduct(productRepository.findById(appointmentDto.getProductId())
                    .orElseThrow(EntityNotFoundException::new));
            appointment.setServiceLength(appointmentDto.getServiceLength());
            appointment.setProduct(productRepository.getReferenceById(appointmentDto.getProductId()));
            appointment.setUser(userRepository.getReferenceById(appointmentDto.getUserId()));
